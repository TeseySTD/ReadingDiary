...
fun ComposeBookItem(
    ...
    // Use 'key' to force recomposition when book changes
    key(book.id) {
        val checked = remember(book.id) { mutableStateOf(false) }
        ...
        Box(
            ...
            Card(
                ...
                Column(
                    ...
                    // Using key with AndroidView to ensure it's recreated correctly
                    key(book.id) {
                        AndroidView(
                            factory = { context ->
                            ...
                            update = { ratingBar ->
                                // Update rating only when this specific book changes
                                ratingBar.rating = book.getRatingValue().toFloat()
